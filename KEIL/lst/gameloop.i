...
void Draw_Sidebar_UI(void) {
  ...
  uint16_t sidebarX1 = 165;
    ...
    // "COMBO"
    lineY = 195;
 LCD_PutString(sidebarX, lineY, (uint8_t *)"COMBO", 0xFFFF, 0x2104);


    // "RESTART"
    lineY = 260;
    LCD_PutString(sidebarX1, lineY, (uint8_t *)"RESTART", 0xFFFF, 0x2104);
...
void LCD_Draw_Border(){
 ...
 LCD_Draw_SquareColumn(15 * 10, 0, 31, 0x7BEF, 0x0000); // x=15 since play area is 14 + 1 spacing

 // Draw top horizontal grey row (under NEXT)
 LCD_Draw_SquareRow(15 * 10, 7 * 10, 8, 0x7BEF, 0x0000); // 7 squares wide

 // Draw bottom horizontal grey row (above RESTART)
 LCD_Draw_SquareRow(15 * 10, 24 * 10, 8, 0x7BEF, 0x0000); // 7 squares wide
 Draw_Sidebar_UI();
}
